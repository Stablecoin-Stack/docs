...
| acquirerId        | A 16-bytes acquirer identifier issued by the Settlement Contract.                                                                                                                                                                                                                              |
| acquirringPrice   | The amount that must be transferred to the Settlement Contract in order to register a new acquirer address, written in the smallest unit of the respective token.                                                                                                                              |
...
| `ref`          | `bytes32` — 66-char hex string | REQUIRED | An opaque 32-byte field, formed by the concatenation of a 16-bytes reference that correlates this transfer to an upstream payment section and the `acquirerId` |
...
9. **Client Wallet** generates a `payloadId` (as described in the section 5.2) and stores it for idem potency tracking.
...
1. **Client Wallet** fetches `acquirringPrice` from the `Settlement Contract`.
2. **Client Wallet** fetches the ERC-2612 nonce for the `owner` address from the token contract.
3. **Client Wallet** constructs `PermitParams` with `owner`, `spender` (settlement contract), `value` equal to `acquirringPrice`, `nonce`, and `deadline`.
